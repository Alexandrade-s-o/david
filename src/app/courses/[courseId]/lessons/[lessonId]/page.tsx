...
function VocabCard({ data }: { data: { word: string; translation: string; pronunciation: string; example: string; audioUrl: string } }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Front */}
        <div className="absolute inset-0 backface-hidden flex flex-col items-center justify-center rounded-3xl border-4 border-[#fbbf24] bg-gradient-to-br from-[#fbbf24] to-amber-600 p-6 shadow-2xl">
          <div className="mb-2 font-display text-4xl font-black uppercase text-[#2e1065]">{data.word}</div>
          <div className="font-handwritten text-xl text-[#2e1065]/80">{data.pronunciation}</div>
          <button
            onClick={(e) => { e.stopPropagation(); /* play audio */ }}
            className="mt-3 rounded-full bg-[#2e1065]/15 p-2 transition-colors hover:bg-[#2e1065]/25"
          >
            <Volume2 className="h-5 w-5 text-[#2e1065]" />
          </button>
          <div className="absolute bottom-3 right-4 text-xs font-black uppercase tracking-wider text-[#2e1065]/70">Tap to flip</div>
        </div>

        <div
          className="absolute inset-0 backface-hidden flex flex-col items-center justify-center rounded-3xl border-4 border-[#fbbf24]/40 bg-[#3b1482] p-6 shadow-card"
          style={{ transform: "rotateY(180deg)" }}
        >
          <div className="mb-2 font-display text-3xl font-black text-white">{data.translation}</div>
          <p className="text-center text-sm font-bold uppercase italic text-white/70">"{data.example}"</p>
        </div>
      </motion.div>
    </motion.div>
  );
...
function MultipleChoiceExercise({
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-display text-lg font-black uppercase tracking-tight text-white md:text-xl">{exercise.question}</h3>
      <p className="text-xs font-black uppercase tracking-wider text-white/55">{exercise.instruction}</p>
            ...
            <motion.button
              ...
              className={`rounded-2xl border-4 p-4 text-left text-sm font-black uppercase tracking-tight transition-all ${
                isCorrect
                  ...
                  : isSelected
                  ? "border-[#fbbf24] bg-[#2e1065] text-[#fbbf24]"
                  : "border-[#fbbf24]/25 bg-[#3b1482] text-white hover:border-[#fbbf24]/60"
              }`}
            >
      ...
      {submitted && (
        ...
        >
          ...
          <p className="text-xs font-bold uppercase leading-relaxed text-white/70">{exercise.explanation}</p>
        </motion.div>
      )}
    </div>
  );
...
export default function LessonPage({
    ...
    return (
      <div className="lpa-page relative flex items-center justify-center p-4">
        ...
        >
          ...
          <h2 className="mb-2 font-display text-3xl font-black uppercase tracking-tight text-white">Lesson complete</h2>
          <p className="mb-8 text-xs font-black uppercase tracking-wider text-white/60">One step closer to the stars</p>
          ...
          <div className="lpa-card mb-8 space-y-4 p-6 text-left">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-black text-[#fbbf24]">{score}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-widest text-white/50">XP earned</span>
              <span className="rounded-full border-2 border-[#fbbf24] px-3 py-1 text-xs font-black text-[#fbbf24]">
                +{xpEarned} XP
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-widest text-white/50">Correct</span>
  ...
  return (
    <div className="lpa-page flex flex-col">
      <header className="sticky top-0 z-40 px-4 py-4 font-bold">
        <div className="mx-auto flex max-w-2xl items-center gap-4 lpa-nav-shell py-3">
          <Link href={`/courses/${courseId}`} className="shrink-0 text-[#fbbf24] transition-colors hover:text-white" aria-label="Back">
            <ArrowLeft className="h-6 w-6" />
          </Link>
          <div className="min-w-0 flex-1">
            <div className="mb-1 truncate text-[10px] font-black uppercase tracking-widest text-white/60">{lesson.title}</div>
          ...
          <div className="flex shrink-0 items-center gap-1 rounded-full border-2 border-[#fbbf24] px-2.5 py-1 text-[10px] font-black text-[#fbbf24]">
        ...
        <AnimatePresence mode="wait">
          ...
          >
              ...
              <div className="space-y-6">
                {currentBlock.type === "story_panel" && (
                  <div className="lpa-card relative overflow-hidden p-8 text-center">
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="font-handwritten text-2xl leading-relaxed text-white">
                ...
                {currentBlock.type === "grammar_rule" && (
                  <div className="lpa-card p-6">
                    ...
                    <h3 className="mb-4 font-display text-lg font-black uppercase tracking-tight text-white">
                      {(currentBlock.data as any).rule}
                    </h3>
                    <ul className="space-y-2">
                      {((currentBlock.data as any).examples as string[]).map((ex: string, i: number) => (
                        <li key={i} className="flex items-start gap-2 text-sm font-bold uppercase italic text-white/75">
                          <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#fbbf24] text-xs font-black text-[#2e1065]">
                            {i + 1}
                          </span>
                          <span>{ex}</span>
                        </li>
            ...
            {!isOnBlock && currentExercise && (
              <div className="lpa-card p-6">
                <div className="mb-6 flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white/50">
                  <span className="rounded-full border-2 border-[#fbbf24]/50 px-2 py-0.5 text-[#fbbf24]">
                    Exercise {exerciseIndex + 1}/{lesson.content.exercises.length}
                  </span>
                  <span className="text-[#fbbf24]">+{currentExercise.xpReward} XP</span>
                </div>
